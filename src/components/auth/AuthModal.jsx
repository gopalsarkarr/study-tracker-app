...
export default function AuthModal() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (!res.success) {
      setClientError(res.error || 'Failed to initialize Google Sign In.');
          ...
          {authModalTab !== 'forgot' && (
            <div className="space-y-2.5">
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl font-bold text-xs bg-white hover:bg-slate-100 text-slate-900 shadow-md transition-all flex items-center justify-center gap-2.5 border border-slate-200 group disabled:opacity-50"
              >
                {isGoogleLoading ? (
                ...
                <span>
                  {authModalTab === 'signup' ? 'Continue with Google (Recommended)' : 'Sign In with Google'}
                </span>
              </button>
              ...
              <div className="relative flex py-1 items-center">
                <div className="flex-grow border-t border-slate-800 dark:border-slate-800 light:border-slate-200"></div>
                <span className="flex-shrink mx-3 text-[10px] uppercase font-mono tracking-wider text-slate-500">
                  Or {authModalTab === 'signup' ? 'Sign Up Manually' : 'with Username / Email'}
                </span>
                <div className="flex-grow border-t border-slate-800 dark:border-slate-800 light:border-slate-200"></div>
              </div>
            </div>
